<?php include "header.php" ?>

<!DOCTYPE html>
<html lang="en">

<!-- Čia galerijos puslapio vidinė dalis -->
<div class="col center-align">
    <div class="card-panel blue lighten-2  z-depth-5">
        <img class="responsive-img" src="https://picsum.photos/id/314/1600/400" alt="nuotrauka">
        <h6>Turizmas (gr. tornos bei lot. tornare, reiškia ratą, keliavimą ratu) – kelionės siekiant pailsėti, patirti naujų įspūdžių ar pasilinksminti. Nors pastaruoju metu terminas labiau reiškia išvykimą iš savo gyvenamosios ar darbinės aplinkos ribų. Masinio turizmo pradininku laikomas Tomas Kukas, baptistų dvasininkas, XIX a. vid. aktyviai kovojęs prieš alkoholizmą. 1841 m. liepos 5 d. jis suorganizavo besigydančių nuo alkoholizmo parapijiečių išvyką į blaivybės šalininkų sambūrį Lafbore (Didžioji Britanija). Dvasininkas subūrė 570 bendraminčių ir pats jiems organizavo kelionę traukiniu bei pasirūpino šių žmonių maitinimu, nakvyne bei pramogomis, siekdamas parodyti, kad pramogos galimos ir be svaigalų. Vėliau T. Kukas ėmėsi naujų žygių – organizavo ekskursijas traukiniais Anglijos varguomenei, vaikams ir darbininkams. Taip atsirado pirmosios turizmo agentūros ir turizmo verslo užuomazgos.

Pamatęs neįtikėtiną susidomėjimą tokio tipo kelionėmis, nuo 1845 m. T. Kukas pradėjo organizuoti ir pirmąsias komercines keliones turistams. Pirmojoje jo surengtoje pramoginėje kelionėje dalyvavo 700 žmonių. T. Kukas vadovaujamos turizmo agentūros klientų skaičius augo, klientai organizuojamomis kelionėmis buvo patenkinti. T. Kukas pasirašinėjo sutartis su geležinkelių, transporto bendrovių, viešbučių, restoranų savininkais, pats asmeniškai sudarinėdavo maršrutus ir ieškodavo kelionių vadovų. Verslas įgijo didesnį mastą 1851 m., kuomet T. Kukas pasiūlė kelionės paketą į Didžiąją Parodą (Great Exhibition) Londone. Už penkis šilingus, vienas asmuo galėjo nukeliauti į parodą, pavalgyti ir gauti nakvynę. Turas susilaukė milžiniškos sėkmės – vien tik Jorkšyro gyventojams parduota virš 165 000 bilietų. Vėliau T. Kukas surengė panašią kelionę į parodą Paryžiuje. Klientams buvo pasiūlytas pilnas paketas – pasų ir vizų sutvarkymas, vertėjas, transportas, maistas, nakvynė ir kelionių čekiai. Netrukus T. Kukas bendrovė pasiūlė turistines keliones traukiniais į daugelį Europos šalių, o 1884 m. surengė kelionę Nilo upe net 18 tūkst. žmonių.  </h6>
    </div>
</div>

<div class="row">
    <div class="col s12 m6 l4 left-align">
        <div class="card-panel blue lighten-4  z-depth-5 ">

            <div class="row valign-wrapper">
                <div class="col s6 m6 l6">
                    <img src="https://picsum.photos/id/164/200/500" alt="" class="responsive-img">
                </div>
                <div class="col s6 m6 l6 left-align">
                    <span class="black-text">
                        <h5> EUROPA </h5>
                        <h6> <a href="about.php"> Susisiekite su mumis </a> dėl kelionės į Europą. </h6>
                        <p>Europa – viena iš pasaulio dalių, kartu su Azija sudaranti Eurazijos žemyną. Europa taip pat dažnai vadinama „Senuoju žemynu“.</p>
                        Daugiau informacijos apie žemyną rasite <a href="https://lt.wikipedia.org/wiki/Europa">čia.</a>

                    </span>

                </div>

            </div>
        </div>
    </div>


    <div class=" col s12 m6 l4 left-align">
        <div class="card-panel grey lighten-2  z-depth-5">
            <div class="row valign-wrapper">
                <div class="col s6 m6 l6">
                    <img src="https://picsum.photos/id/686/200/500" alt="" class="responsive-img">
                </div>
                <div class="col s6 m6 l6 left-align">
                    <span class="black-text">
                        <h5> AZIJA </h5>
                            <h6> <a href="about.php"> Susisiekite su mumis </a> dėl kelionės į didingą Aziją. </h6>
                                <p>Azija – didžiausia pasaulio dalis, čia gyvena apie 60 % pasaulio gyventojų. Su Europa sudaro Eurazijos žemyną.
                        </p>
                                        Daugiau informacijos apie žemyną rasite <a href="https://lt.wikipedia.org/wiki/Azija"" >čia.</a>
                    </span>

                </div>

            </div>
        </div>
    </div>



    <div class=" col s12 m6 l4 left-align">
            <div class="card-panel blue lighten-4  z-depth-5">
                 <div class="row valign-wrapper">
                     <div class="col s6 m6 l6">
                             <img src="https://picsum.photos/id/283/200/500" alt="" class="responsive-img">
                    </div>
                    <div class="col s6 m6 l6 left-align">
                         <span class="black-text">
                             <h5> ŠIAURĖS AMERIKA </h5>
                             <h6> <a href="about.php"> Susisiekite su mumis </a> dėl kelionės į Šiaurės Ameriką. </h6>
                                    <p>Šiaurės Amerika – trečias pagal plotą žemynas, esantis šiaurės pusrutulyje.
                                        </p>
                                             Daugiau informacijos apie žemyną rasite <a href="https://lt.wikipedia.org/wiki/%C5%A0iaur%C4%97s_Amerika"> čia.</a>
                            </span>

                     </div>

                    </div>
             </div>
     </div>





     <div class="col s12 m6 l4 left-align">
         <div class="card-panel grey lighten-2  z-depth-5">

              <div class="row valign-wrapper">
                 <div class="col s6 m6 l6">
                                <img src="https://picsum.photos/id/174/200/500" alt="" class="responsive-img">
                 </div>
                 <div class="col s6 m6 l6 left-align">
                         <span class="black-text">
                         <h5> PIETŲ AMERIKA </h5>
                         <h6> <a href="about.php"> Susisiekite su mumis </a> dėl kelionės į Pietų Ameriką. </h6>
                                <p>Pietų Amerika – didžiąja dalimi yra pietiniame Žemės pusrutulyje. Palei vakarinį žemyno pakraštį driekiasi Andų kalnų grandinė. 
                                             </p>
                                         
                                         Daugiau informacijos apie žemyną rasite <a href="https://lt.wikipedia.org/wiki/Piet%C5%B3_Amerika">čia.</a>

                         </span>

                 </div>

             </div>
        </div>
    </div>



                <div class="col s12 m6 l4 left-align">
                    <div class="card-panel blue lighten-4  z-depth-5">
                        <div class="row valign-wrapper">
                            <div class="col s6 m6 l6">
                                <img src="https://picsum.photos/id/1016/200/500" alt="" class="responsive-img">
                            </div>
                            <div class="col s6 m6 l6 left-align">
                                <span class="black-text">
                                    <h5> AFRIKA <h5>
                                    <h6> <a href="about.php"> Susisiekite su mumis </a> dėl kelionės į Afriką. </h6>
                                <p>Afrika – antrasis pagal dydį (po Azijos) žemynas. Jame gyvena septintadalis Žemės gyventojų (800 milijonų žmonių). 
                                         </p>
                                         Daugiau informacijos apie žemyną rasite <a href="https://lt.wikipedia.org/wiki/Afrika">čia.</a>
                                </span>

                            </div>

                        </div>
                    </div>
                </div>



                <div class="col s12 m6 l4 left-align">
                    <div class="card-panel grey lighten-2  z-depth-5">
                        <div class="row valign-wrapper">
                            <div class="col s6 m6 l6">
                                <img src="https://picsum.photos/id/1015/200/500" alt="" class="responsive-img">
                            </div>
                            <div class="col s6 m6 l6 left-align">
                                <span class="black-text">
                                    <h5> AUSTRALIJA <h5>
                                <h6> <a href="about.php"> Susisiekite su mumis </a> dėl kelionės į Australiją. </h6>
                                            <p>Australija – šešta pagal dydį pasaulio šalis. „Australijos“ pavadinimas kilęs iš lot. terra australis incognita (nežinoma pietų žemė).
                                         </p>
                                            
                                         Daugiau informacijos apie žemyną rasite <a href="https://lt.wikipedia.org/wiki/Australija">čia.</a>

                                </span>

                            </div>

                        </div>
                    </div>
                </div>
</div>
   
<?php include "footer.php" ?>